...
def train_model(model, train_dataset, valid_dataset, num_epochs, output_dir="./llm_models", bestmodel_dir="best_llm_model"):
...
def create_model():
    ...
    with open("train_sva_data.json", "r", encoding="utf-8") as fp:
        train_data = json.load(fp)
    train_size = len(train_data)
    epoch = 1
   
    # Load training, valid, and test dataset as polars dataframes
    pl_train = pl.read_json("train_sva_data.json").to_pandas()
    pl_train = decide_train_size(pl_train, train_size) 
    pl_valid = pl.read_json("valid_sva_data.json").to_pandas()
    ...
    # Train the model
    train_model(model, train_dataset, valid_dataset, epoch, "./llm_models", "best_llm_model")
...
def test_created_model():
    pl_test = pl.read_json("test_sva_data.json")
    model_dir = "./llm_models/best_llm_model"
